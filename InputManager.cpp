...
void InputManager::InputCycle(sf::RenderWindow* targetWindow, int enumState)
{
	if (enumState == mainMenu)
	{
		...
		if (sf::Keyboard::isKeyPressed(sf::Keyboard::Down))
		{
			if (!downKeyDown)
			{
				if (Game.GetMap() < Game.mapList.size() - 1)
					Game.SwitchMapSel(1);
				downKeyDown = true;
			}
		}
		...
		{
			MapMngr.LoadMap("maps/" + Game.mapList[Game.GetMap()]);
			MapDrwr.InitMapTexture(targetWindow, MapMngr.GetMap(), MapMngr.GetMapHeight(), MapMngr.GetMapWidth());
			Game.SwitchState(mapMode);
			Tree.InitTexts();
		}
	}

	if (enumState == mapMode)
	{
